      ...
      <section className="surface p-5 sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="status-chip">Fonte: {source === 'api' ? 'backend REST' : source === 'mock' ? 'mock local' : 'sem dados'}</span>
            <span className="rounded-lg border border-border bg-background/80 px-3 py-2 text-xs font-semibold text-foreground">
              GET {module.endpoint}
            </span>
          </div>
        </div>
      ...
      <section className="surface p-5 sm:p-6">
        ...
        {source === 'mock' ? (
          <div className="mb-4 rounded-lg border border-accent/30 bg-accent/10 px-4 py-3 text-sm text-accent-foreground">
            {sourceMessage ?? 'Exibindo dados locais de demonstracao ate a API retornar registros.'}
          </div>
        ) : null}
            ...
            <table className="data-table min-w-full text-sm">
              ...
              <tbody>
                {filteredRows.map((row, index) => (
                  <tr key={`${module.key}-row-${index}`} className="border-b border-border/70 bg-card transition hover:bg-background/75">
                    {columns.map((column) => (
